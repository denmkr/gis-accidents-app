...
class AverageBardensityLayer extends Overlay {
  constructor(name: string, featureCollection: FeatureCollection) {
    super(name, featureCollection);
  }
  ...
  createOverlay() {
    ...
    const colorScale = d3.scaleSequential(d3.interpolatePuOr).domain(minMaxBars);
...
export {Overlay, LandkreisLayer, BardichteLayer, AverageBardensityLayer};
